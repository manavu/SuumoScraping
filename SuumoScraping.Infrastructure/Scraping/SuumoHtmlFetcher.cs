    ...
    public class SuumoHtmlFetcher : ISuumoHtmlFetcher
    {
            ...
            for (var i = 0; i < maxRetry; i++)
            {
                try
                {
                    this._logger.LogInformation(
                        "HTML取得開始: {Url} (試行 {RetryCount}/{MaxRetry})",
                        url,
                        i + 1,
                        maxRetry
                    );
                ...
                catch (HttpRequestException e)
                {
                    this._logger.LogWarning(
                        e,
                        "HTML取得に失敗しました (HttpRequestException): {Url}. メッセージ: {Message}",
                        url,
                        e.Message
                    );
                    ...
                    {
                        throw new SuumoFetchException(
                            "Suumoサーバーで内部エラーが発生しました。",
                            url,
                            (int?)e.StatusCode,
                            e
                        );
                ...
                catch (Exception e)
                {
                    this._logger.LogWarning(
                        e,
                        "予期せぬエラーによりHTML取得に失敗しました: {Url}. メッセージ: {Message}",
                        url,
                        e.Message
                    );
                    ...
                    {
                        throw new SuumoFetchException(
                            "リトライ上限に達したため、予期せぬエラーによりHTML取得処理を断念しました。",
                            url,
                            null,
                            e
                        );
        ...
        )
        {
            ...
            for (var i = 0; i < maxRetry; i++)
            {
                ...
                {
                    this._logger.LogInformation(
                        "バイナリデータ取得開始: {Url} (試行 {RetryCount}/{MaxRetry})",
                        url,
                        i + 1,
                        maxRetry
                    );
                ...
                catch (HttpRequestException e)
                {
                    this._logger.LogWarning(
                        e,
                        "バイナリ取得に失敗しました (HttpRequestException): {Url}. メッセージ: {Message}",
                        url,
                        e.Message
                    );
                    ...
                    {
                        throw new SuumoFetchException(
                            "リトライ上限に達したため、バイナリデータ取得処理を断念しました。",
                            url,
                            (int?)e.StatusCode,
                            e
                        );
                ...
                catch (Exception e)
                {
                    this._logger.LogWarning(
                        e,
                        "予期せぬエラーによりバイナリ取得に失敗しました: {Url}. メッセージ: {Message}",
                        url,
                        e.Message
                    );
                    ...
                    {
                        throw new SuumoFetchException(
                            "リトライ上限に達したため、予期せぬエラーによりバイナリデータ取得処理を断念しました。",
                            url,
                            null,
                            e
                        );
            ...
            throw new SuumoFetchException(
                "バイナリデータ取得処理が正常に終了しませんでした（不明な理由）。",
                url
            );
        }
