namespace SuumoScraping.Infrastructure.Scraping
{
    ...
    public class SuumoGateway : ISuumoGateway
    {
        ...
        public SuumoGateway(ISuumoHtmlFetcher fetcher, ISuumoHtmlParser parser, ILogger<SuumoGateway> logger)
        {
        ...
        public async Task<AreaPageResult> GetAreaPageAsync(string url, CancellationToken cancellationToken = default)
        {
            try
            {
                this._logger.LogInformation("エリア一覧取得処理開始: {Url}", url);
                var html = await this._fetcher.GetHtmlStringAsync(url, cancellationToken).ConfigureAwait(false);
                ...
                this._logger.LogInformation("エリア一覧取得処理成功: {Url}. 取得件数: {Count}件", url, result.Bukkens.Count);
                return result;
            }
            ...
            {
                this._logger.LogError(e, "エリア一覧取得処理中に予期せぬエラーが発生しました: {Url}", url);
                throw new SuumoScrapingException("エリア一覧の取得に失敗しました。", url, e);
            }
        }

        public async Task<ScrapedBukkenDetail> GetBukkenDetailAsync(string detailUrl, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var gaiyoHtml = await this._fetcher.GetHtmlStringAsync(gaiyoUrl, cancellationToken).ConfigureAwait(false);

                if (string.IsNullOrEmpty(gaiyoHtml))
                {
                    throw new SuumoParseException("取得した物件概要HTMLが空です。", gaiyoUrl, "bukkengaiyo_html");
                }

                // 物件特徴ページ
                this._logger.LogInformation("物件特徴フェッチ開始: {Url}", detailUrl);
                var tokuchoHtml = await this._fetcher.GetHtmlStringAsync(detailUrl, cancellationToken).ConfigureAwait(false);
            ...
            {
                this._logger.LogError(e, "物件詳細取得・解析中に予期せぬエラーが発生しました: {Url}", detailUrl);
        ...
        public async Task<byte[]> GetFileDataAsync(string url, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var data = await this._fetcher.GetFileDataAsync(url, cancellationToken).ConfigureAwait(false);
                this._logger.LogInformation("ファイルダウンロード成功: {Url}. サイズ: {Size}bytes", url, data?.Length ?? 0);
                return data;
            }
            catch (SuumoScrapingException)
            {
            ...
            {
                this._logger.LogError(e, "ファイルダウンロード中に予期せぬエラーが発生しました: {Url}", url);
